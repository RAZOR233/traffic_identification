...
class BasicBlock(paddle.nn.Layer):
    def __init__(self, in_channels, out_channels, kernel_size=[3, 2],
        ...
        self.conv1 = Conv2D(in_channels, in_channels, kernel_size=kernel_size[0],
                            stride=stride, padding=padding[0], dilation=dilation[0])
        self.bn1 = BatchNorm2D(in_channels)
        self.conv2 = Conv2D(in_channels, out_channels, kernel_size=kernel_size[1],
                            stride=stride, padding=padding[1], dilation=dilation[1])
        self.bn2 = BatchNorm2D(out_channels)
        self.conv3 = Conv2D(out_channels, out_channels, kernel_size=kernel_size[1],
                            stride=stride, padding=padding[1], dilation=dilation[1])
...
class CRNN(paddle.nn.Layer):
    def __init__(self, in_size, in_channels, num_classes, num_hidden=64, num_rnn=2):
        super(CRNN, self).__init__()

        cnn = paddle.nn.Sequential(
            BasicBlock(in_channels, 64),  # 64*128*128
            MaxPool2D(4),  # 64*32*32
            BasicBlock(64, 128),  # 128*32*32
            MaxPool2D((4,1)),  # 128*8*32
            BasicBlock(128, 256),  # 256*8*32
            MaxPool2D((2, 1)),  # 256*4*32
            BasicBlock(256, 512),  # 512*4*32
            MaxPool2D((4, 1)),  # 512*1*32
            BatchNorm2D(512) # 512*1*32
        )
        self.cnn = cnn
        ...
        self.fc1 = Linear(32 * num_hidden * 2, 64)
    ...
    def forward(self, x):
        # conv features
        conv = self.cnn(x)
        conv = paddle.squeeze(conv, 2)
        conv = paddle.transpose(conv, perm=[0, 2, 1])
        # rnn features
        bi_lstm = self.rnn(conv)
